...
object CoverTypeDatasetApp{
  def main (arg: Array[String]): Unit = {
    ...
    // Load Data
    var training = spark.read.option("header","true").option("inferSchema","true").format("csv").load("wasb:///example/data/Cover_Type_training_data.csv")
    var testing  = spark.read.option("header","true").option("inferSchema","true").format("csv").load("wasb:///example/data/Cover_Type_Testing_data.csv")//("/home/eissa/mycode/data/testing_data.csv")
    print("Data Laoded " +  "\n")

    // Process data
    training = training.drop("_c0")
    val featureCols = training.columns.filter(_ != "Label")
